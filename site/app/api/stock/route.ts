...
export async function POST(request: Request) {
  ...
  try {
    ...
    let sourceMetadata: Record<string, { id?: string; modifiedTime?: string }> = {};
    try {
      sourceMetadata = JSON.parse(String(formData.get("sourceMetadata") ?? "{}"));
    } catch {
      return Response.json({ error: "Metadados de origem inválidos." }, { status: 400 });
    }
    const sourceKind =
    ...
    for (const file of files) {
      ...
      parsed.push({
        ...
        buffer,
        rows,
        sourceId: sourceMetadata[file.name]?.id ?? null,
        sourceModifiedAt: sourceMetadata[file.name]?.modifiedTime ?? null,
      });
    }
